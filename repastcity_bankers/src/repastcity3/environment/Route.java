...
public class Route implements Cacheable {
		...
		try {
			...
			while (!travelledMaxDist && !this.atDestination()) {
				...
				// distance allowed to travel (unlikely but possible)
					...
					myDistance+=distTravelled + distToTarget;
					this.currentPosition++;
					if (GlobalVars.loggerOn) {
					...
					// speed)
					double distToTravel = (GlobalVars.GEOGRAPHY_PARAMS.TRAVEL_PER_TURN - distTravelled) * speed; //[in meters] [ 1 tick=
	...
	private double distance(Coordinate c1, Coordinate c2, double[] returnVals) {
		...
		double distance;
		try {
			distance = calculator.getOrthodromicDistance(); // 	getOrthodromicDistance(). Returns the orthodromic distance (expressed in meters).
	...
	/**
	 * Get the distance from the agent's current position to the given destination.
	 * 
	 * @param agent The agent for whom the distances will be calculated
	 * @param destination The destination to calculate a route to.
	 * @param destBuilding The destination building
	 * @return A double array with two elemts. The first is the distance, the second is the travel time taking
	 * into account of the transport available to the agent. 
	 * @author Nick Malleson
	 * @throws Exception
	 */

	//	Nicks way
	public static double[] getRouteDistance	(IAgent agent, Coordinate destination, Building destBuilding) throws Exception {
		// Create a new Route object and calculate how to get there
		Route r = new Route(agent, destination, destBuilding);
		r.setRoute();

		// Now just iterate through the coordinates that make up the route and add up the total distance and
		// travel time.
		double totalDist = 0.0, totalTime = 0.0, dist = 0.0, speed = 0.0,  myCheck=0.0; //myCheck is added
		GeodeticCalculator calculator = new GeodeticCalculator(ContextManager.roadProjection.getCRS()); // For distance calc

		for (int i=0; i<r.routeX.size()-1; i++) {
			Coordinate c1 = r.routeX.get(i); // Current coordinate
			Coordinate c2 = r.routeX.get(i+1); // The next one in the route
			speed = r.routeSpeedsX.get(i); // The speed between the two
			calculator.setStartingGeographicPoint(c1.x, c1.y);
			calculator.setDestinationGeographicPoint(c2.x, c2.y);
		...
		if (GlobalVars.loggerOn) {
			LOGGER.log(Level.FINE, "Route.getRouteDistance calculated route distance for agent "+agent.toString() +" :\n\t"
					+ "Destination (coord,building):" + destination.toString() +", " + destBuilding.toString() + "\n\t"
					+ "Distance: (dist,time)" + totalDist +"," + totalTime + " my check: "+myCheck/100 +"  "+speed + "  "+dist);
		}
		return new double[]{totalDist, totalTime};
	}
}
