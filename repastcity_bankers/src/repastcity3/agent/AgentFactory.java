...
public class AgentFactory {
	...
	private void createRandomAgents(boolean dummy) throws AgentCreationException {
		...
		int agentWithBus=-1;
		int allAgents=0;
		//here we run through every building that is home and we read its number of agents we want to create
		for (Building b : ContextManager.buildingContext.getObjects(Building.class)) {
			if (b.getType()==1){
				
				if(GlobalVars.mySport=="swimming"){
					agentWithCars= b.getAgentsCarS();
					agentWithBus=b.getAgentsBusS();
				}
				else if(GlobalVars.mySport=="fitness"){
					agentWithCars= b.getAgentsCarF();
					agentWithBus=b.getAgentsBusF();
				}
				
				int agentsCreated = 0;				
				while (agentsCreated < agentWithCars){
					IAgent a = new DefaultAgent(); // Create a new agent
					a.setHome(b); // Tell the agent where it lives
					
					a.setTransport("car walk");//set his mode of transport
					a.setZONE_ID((double) b.GetZONE_ID()); //set ZONE_ID
				...
				allAgents+=agentsCreated;
				
				agentsCreated = 0;
				while (agentsCreated < agentWithBus){
					IAgent a = new DefaultAgent(); // Create a new agent
					a.setHome(b); // Tell the agent where it lives
					
					a.setTransport("bus walk");//set his mode of transport
					a.setZONE_ID((double) b.GetZONE_ID());//set ZONE_ID
